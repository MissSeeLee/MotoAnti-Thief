  ...
  <div class="h-full bg-[#0f172a] text-slate-300 flex flex-col font-sans overflow-hidden border-r border-slate-800 shadow-2xl relative z-40">
    ...
    <div class="p-6 pb-4 flex items-center gap-4 bg-[#0f172a] z-10">
      <div @click="openProfileModal" class="relative group cursor-pointer flex items-center gap-4 w-full hover:bg-slate-800/50 p-2 -ml-2 rounded-xl transition-all">
        <div class="relative">
          <div class="w-12 h-12 rounded-2xl bg-slate-800 flex items-center justify-center text-slate-400 ring-1 ring-slate-700/50 shadow-lg group-hover:ring-blue-500/50 transition-all">
    ...
    <div class="flex-1 overflow-y-auto px-4 py-2 space-y-3 custom-scrollbar">
       <div v-for="(device, key) in devices" :key="key" @click="$emit('select-device', device.deviceId || device.id)" class="relative group rounded-2xl p-3 transition-all duration-300 border cursor-pointer overflow-hidden" :class="isActive(device) ? 'bg-blue-600/10 border-blue-500/50' : 'bg-[#1e293b]/40 border-slate-700/30 hover:bg-[#1e293b]'">
           
           <button v-if="isOwner" @click.stop="$emit('delete-device', device)" class="absolute top-2 right-2 z-30 p-1.5 rounded-full text-slate-500 bg-slate-800/50 hover:bg-rose-500 hover:text-white transition-all"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor" class="w-3 h-3"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" /></svg></button>
           ...
           <div class="flex items-start gap-3 mb-3 pl-2 relative z-10">
              <div class="w-10 h-10 rounded-xl flex items-center justify-center text-xl shadow-inner transition-all duration-300" :class="isActive(device) ? 'bg-gradient-to-br from-blue-500 to-indigo-600 text-white' : 'bg-slate-800 text-slate-500'">🛵</div>
              <div class="flex-1 min-w-0 pt-0.5 pr-6"> 
                 <h3 class="font-bold text-sm truncate" :class="isActive(device) ? 'text-white' : 'text-slate-300'">{{ device.name || 'Unknown Device' }}</h3>
...
import api from '../api';
import ProfileModal from './ProfileModal.vue'; // ✅ Import เข้ามา
...
const emit = defineEmits(['select-device', 'logout', 'add-device', 'delete-device', 'edit-device', 'open-geofence', 'view-history', 'find-bike', 'share-device', 'toast']);

const isProfileModalOpen = ref(false);
const userProfile = reactive({ name: '', email: '', phone: '' });

const isActive = (device) => (device.deviceId || device.id) === props.activeDeviceId;

const fetchUserProfile = async () => {
  const storedUser = localStorage.getItem('user');
  if (storedUser) {
    const u = JSON.parse(storedUser);
    Object.assign(userProfile, { name: u.name || u.username, email: u.email || '', phone: u.phone || '' });
  }
  try {
...
const handleProfileSaved = (u) => Object.assign(userProfile, { name: u.name, email: u.email, phone: u.phone });
